...
class PusherEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release.
    ...
    def step(self, action):
        ...
        # Verifica se il numero di passi ha raggiunto il limite
        truncation = self.step_count >= 75
    ...
    def _get_rew(self, action):
        # Ottieni le coordinate del centro di massa del r_elbow_flex_link, dell'oggetto e del goal
        self.goal_achievement = False
        elbow_pos = self.get_body_com("end_effector")
        object_pos = self.get_body_com("object")
        goal_pos = self.get_body_com("goal")

        # Calcola le distanze
        distance_elbow_to_object = np.linalg.norm(elbow_pos - object_pos)
        distance_object_to_goal = np.linalg.norm(object_pos - goal_pos)
        ...
        reward = - (distance_elbow_to_object + distance_object_to_goal) + 2 * delta_distance_object_to_goal  + static_penalty
        
        if object_pos[0] > 6 or object_pos[1] > 3 or object_pos[2] > 6:
            self.goal_achievement = True
        # Aggiorna la distanza precedente
        self.previous_distance_object_to_goal = distance_object_to_goal
        
        if distance_object_to_goal < 2.01:
            reward += 100
            self.goal_achievement = True
    ...
    def _get_obs(self):
        return np.concatenate(
            [
                ...
                self.get_body_com("end_effector"),
